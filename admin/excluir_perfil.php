...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['perfil_id'])) {
    ...
    if ($perfil_id) {
        ...
        // Busca info do perfil (garantindo que pertence a esta prefeitura)
        $stmt_perfil = $pdo->prepare("SELECT nome FROM perfis WHERE id = ? AND (id_prefeitura = ? OR id_prefeitura IS NULL)");
        $stmt_perfil->execute([$perfil_id, $id_prefeitura]);
        ...
        if ($perfil) {
            ...
            if ($perfil_id == 1 || strtolower($nome_perfil) === 'administrador') {
                ...
                // Verifica se há usuários vinculados (nesta prefeitura especificamente)
                $stmt_users = $pdo->prepare("SELECT COUNT(*) FROM usuarios_admin WHERE id_perfil = ? AND id_prefeitura = ?");
                $stmt_users->execute([$perfil_id, $id_prefeitura]);
                ...
                if ($total_usuarios > 0 && (!$transferir_id || $transferir_id == $perfil_id)) {
                    ...
                    try {
                        ...
                        if ($total_usuarios > 0 && $transferir_id) {
                            $stmt_transfer = $pdo->prepare("UPDATE usuarios_admin SET id_perfil = ? WHERE id_perfil = ? AND id_prefeitura = ?");
                            $stmt_transfer->execute([$transferir_id, $perfil_id, $id_prefeitura]);
